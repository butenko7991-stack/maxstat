...
export default function AdminPage() {
  ...
  const [newInvite, setNewInvite] = useState({ role: "buyer" as "admin" | "buyer" | "manager" });
  ...
  const openInviteDialog = () => {
    setNewInvite({ role: user?.role === "owner" ? "admin" : "buyer" });
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="users" className="mt-4 space-y-3">
          ...
          <div className="mt-5 border-t border-border pt-4"><h2 className="text-sm font-semibold text-foreground">Приглашения</h2><p className="mt-1 text-xs text-muted-foreground">Ссылка одноразовая, действует 7 дней и не содержит доступ к вашей учётной записи.</p><div className="mt-3 space-y-2">{invitations && invitations.length > 0 ? invitations.map((invitation) => {
            ...
            return <div key={invitation.id} className="glass flex flex-wrap items-center justify-between gap-3 rounded-xl p-3"><div className="min-w-0"><p className="truncate text-sm font-medium text-foreground">Ссылка-приглашение</p><p className="text-xs text-muted-foreground">{ROLE_LABELS[invitation.role]} · {status} · до {new Date(invitation.expiresAt).toLocaleDateString("ru-RU")}</p></div>{active && <Button variant="ghost" size="sm" className="h-8 text-xs text-red-400 hover:bg-red-500/10 hover:text-red-300" disabled={revokeInvitationMutation.isPending} onClick={() => revokeInvitationMutation.mutate({ invitationId: invitation.id })}><X className="mr-1 h-3.5 w-3.5" />Отозвать</Button>}</div>;
