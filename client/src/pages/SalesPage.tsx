...
export default function SalesPage() {
  ...
  const { data: records, isLoading } = trpc.sales.list.useQuery(listInput);
  ...
  const { data: exportData, isFetching: exportFetching, refetch: refetchExport } =
    trpc.sales.exportData.useQuery(
      { month: selectedMonth !== "all" ? selectedMonth : undefined,
        channelId: selectedChannel !== "all" ? Number(selectedChannel) : undefined },
      { enabled: false }
    );
  ...
  const quickPayMutation = trpc.sales.quickUpdatePayment.useMutation({
    onMutate: async ({ id, paymentStatus }) => {
      await utils.sales.list.cancel(listInput);
      const prev = utils.sales.list.getData(listInput);
      utils.sales.list.setData(listInput, (old) =>
        old?.map((r) => r.id === id ? { ...r, paymentStatus } : r)
      );
      return { prev };
    },
    onError: (_e, _v, ctx) => {
      if (ctx?.prev) utils.sales.list.setData(listInput, () => ctx.prev);
  ...
  useEffect(() => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Продажа");
    const monthLabel = selectedMonth !== "all" ? `_${selectedMonth}` : "";
    XLSX.writeFile(wb, `Продажа${monthLabel}.xlsx`);
    setExportPending(false);
    ...
    <div className="p-4 lg:p-6 space-y-5">
      ...
      <div className="relative">
        ...
        <Input
          ...
          placeholder="Поиск по админу, ссылке, платформе..."
