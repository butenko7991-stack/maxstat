...
## Креативы каналов
- [x] Добавить модель креатива, привязанного к каналу и рабочей зоне
- [x] Реализовать загрузку скриншота в файловое хранилище и сохранение текста рекламного поста
- [x] Добавить блок «Креативы» в раскрывающиеся карточки каналов с созданием и удалением
- [x] Покрыть доступ и CRUD креативов тестами, собрать и развернуть на VPS

## Сопоставление отчётов с креативами
- [x] Сопоставлять текст и превью отчёта с сохранёнными креативами каналов
- [x] Выбирать канал и охват за 24 часа автоматически только при единственном надёжном совпадении
- [x] Использовать креативы при ручном извлечении ссылок и в массовой коррекции
- [x] Покрыть правила совпадения тестами, собрать и развернуть на VPS

## Исправление миграции распознанного текста
- [x] Создать совместимое поле `recognizedText` для уже развёрнутой таблицы креативов
- [x] Проверить, что анализ ссылок не падает при чтении сохранённых креативов
- [x] Развернуть исправление на VPS и повторно проверить массовую коррекцию

## Граница применения креативов
- [x] Применять сопоставление креативов только при анализе закупов рекламы
- [x] Сохранить для продаж обычный выбор охватов по данным отчёта без креативов
- [x] Покрыть разделение сценариев тестом и развернуть исправление на VPS

## Исключение корректных ссылок из повторной проверки
- [x] Сохранять подтверждённый статус совпадения охвата за 24 часа для записи
- [x] Исключать уже корректные записи из новых запусков массовой коррекции
- [x] Покрыть фильтрацию тестами и развернуть оптимизацию на VPS

## Срочное восстановление схемы проверки охватов
- [x] Создать отсутствующие поля проверки в существующих таблицах VPS до чтения записей
- [x] Подтвердить отсутствие ошибки загрузки списка коррекции
- [x] Добавить защиту от повторного рассинхрона схемы и развернуть исправление

## ВП при создании продажи
- [x] Сравнить передачу флага ВП при первичном создании закупа и продажи
- [x] Исправить сохранение ВП при первом создании продажи без повторного редактирования
- [x] Добавить регрессионный тест, проверить сборку и развернуть исправление на VPS

## Прямой запуск массовой коррекции
- [x] Подготовить изолированное обновление только рабочей зоны продажи #4950161
- [x] Применить однозначно подтверждённые охваты за 24 часа и сохранить итоговый отчёт: обновлено 94, без изменений 502, оставлено на проверку 29, ошибки 2
- [x] Удалить одноразовый запуск после завершения и подтвердить состояние приложения
...
## ВП в расписании (взаимки в сетке)
- [x] Backend: расширить getScheduleData — добавить mutual deals за неделю (по dealDate)
...
## Мультивыбор каналов в ВП

- [ ] Обновить схему mutual_deals: добавить поле ourChannelIds (JSON-массив) рядом с ourChannelId
- [ ] Обновить DB-хелперы и tRPC router mutual для поддержки нескольких каналов
- [ ] Обновить форму MutualPage: мультивыбор каналов (чекбоксы/теги) вместо одного select
- [ ] Обновить список ВП-сделок: отображать несколько каналов в карточке
- [ ] Тесты и checkpoint

## Недельный анализ работы

- [ ] Backend: tRPC процедура ai.weeklyStats — продажи/закуп/расходы/прибыль за текущую и прошлую неделю
- [ ] Backend: tRPC процедура ai.weeklyAnalysis — AI-анализ недели с мотивирующими рекомендациями
- [ ] Frontend: вкладка «Неделя» в AIAnalyticsPage — KPI-карточки, тренды, AI-рекомендации

## Исправление ВП в продажах
- [x] Передавать актуальное isMutual при создании и редактировании продажи через расписание

## Аудит и развитие продукта
- [x] Провести глубокий аудит функций, данных, UX, аналитики, VPS и безопасности; подготовить приоритетную дорожную карту

## Первый спринт — устойчивость и защита
- [x] Подтвердить и закрепить исправление ВП при создании и редактировании продаж
- [x] Подключить максадмин.рф к VPS, включить HTTPS и ограничить прямой доступ к Node.js
- [x] Настроить независимое хранение бэкапов в Google Drive и провести тестовое восстановление
- [x] Настроить единый выпуск из GitHub с health-check и безопасным rollback
- [x] Исправить порядок установки pnpm, убрать конфликт версий и добавить тестовый JWT_SECRET в CI
- [x] Создать BEGET_SSH_KEY и подтвердить успешный SSH-деплой GitHub Actions

## Многопользовательская версия
- [x] Зафиксировать владельца с неизменяемым полным доступом
- [x] Добавить безопасную регистрацию пользователей и создание администраторов владельцем
- [x] Реализовать независимые рабочие зоны: каждый админ видит и ведёт только свои каналы, учёт и аналитику
- [x] Реализовать роли закупщика и менеджера только внутри команды конкретного администратора
- [x] Создать интерфейс владельца и администраторов для регистрации пользователей и управления своими командами
- [x] Покрыть регистрацию и разграничение доступа тестами, задеплоить на VPS

## Ошибка AI-аналитики
- [x] Убрать обращение к отсутствующему полю channels.isVisible и проверить запуск AI-анализа

## ВП при создании продажи
- [x] Исправить сохранение isMutual при создании новой продажи и проверить на VPS
- [x] Повторно подтвердить сохранение ВП-продажи без внесения тестовых данных в рабочую базу

## Глубокий аудит после многопользовательского режима
- [x] Проверить ошибки бизнес-логики, изоляцию данных, авторизацию и уязвимости
- [x] Проверить VPS, HTTPS, резервные копии, GitHub Actions и эксплуатационные риски
- [x] Подготовить приоритетную дорожную карту необходимых исправлений и улучшений

## P0 — доступ сотрудников только к назначенным каналам
- [x] Ввести серверную проверку назначений channelAssignments для buyer и manager
- [x] Закрыть операции чтения и изменения чужих каналов, продаж и закупок
- [x] Добавить тест-матрицу разрешённых и запрещённых операций, задеплоить на VPS

## Охваты при смене статуса оплаты
- [x] Проверить историю и источник охватов, автоматически записываемых при смене статуса на «Оплачено»
- [x] Исправить недостоверный выбор значения охватов и добавить регрессионные тесты
- [x] Развернуть исправление на VPS и подтвердить работу страницы расписания

## Массовая корректировка исторических охватов
- [x] Добавить предпросмотр оплаченных записей со ссылками и классификацией: обновить / пропустить / проверить вручную
- [x] Реализовать пакетное обновление только однозначно сопоставленных охватов за 24 часа
- [x] Показать пользователю результаты обработки и список спорных записей без изменения данных
- [x] Покрыть правила массового обновления тестами и развернуть на VPS

## Ссылки аналитика-мах.рф
- [x] Проанализировать ответ go.аналитика-мах.рф и причину отсутствия охватов
- [x] Добавить извлечение охватов за 24 часа для ссылок аналитики
- [x] Покрыть формат ссылки регрессионным тестом и развернуть исправление на VPS

## Уведомление об обновлении
- [x] Показать пользователю всплывающее уведомление о точном извлечении охватов из ссылок Аналитики МАХ
- [x] Запоминать закрытие уведомления на текущей версии и дать ссылку на массовую коррекцию охватов
- [x] Покрыть логику уведомления тестом и развернуть на VPS

## Ошибки массовой коррекции охватов
- [x] Исправить ошибку `ByteString` для ссылок с кириллическим доменом
- [x] Уточнить обработку старых отчётов без зафиксированного значения за 24 часа
- [x] Покрыть исправления регрессионными тестами и развернуть на VPS

## Сохранённые охваты старых отчётов
- [x] Получить реальные ссылки записей, отмеченных как «Нет данных 24ч», и сверить JSON-ответы с отображением сервиса
- [x] Расширить извлечение сохранённого показателя за 24 часа для фактического формата отчёта
- [x] Добавить тест реального формата и развернуть исправление на VPS

## Контрольная ссылка Аналитики МАХ
- [x] Воспроизвести ошибку на ссылке `ad_-70945865509462_1786818548541_1j8v7l` и проверить прямой JSON API без заголовка Referer

## Список массовой коррекции
- [x] Исключить строки со статусом «Уже верно» из предпросмотра и счётчиков коррекции
- [x] Покрыть фильтрацию тестом и развернуть изменение на VPS
